...
func BaremetalAPIWithZoneAndID(m interface{}, id string) (*baremetal.API, locality.ZonedID, error) {
	meta := m.(*meta2.Meta)
	baremetalAPI := baremetal.NewAPI(meta.GetScwClient())

	zone, ID, err := locality.ParseZonedID(id)
	if err != nil {
		return nil, locality.ZonedID{}, err
	}
	return baremetalAPI, locality.NewZonedID(zone, ID), nil
}
...
func BaremetalPrivateNetworkAPIWithZoneAndID(m interface{}, id string) (*baremetal.PrivateNetworkAPI, locality.ZonedID, error) {
	meta := m.(*meta2.Meta)
	baremetalPrivateNetworkAPI := baremetal.NewPrivateNetworkAPI(meta.GetScwClient())
	...
	return baremetalPrivateNetworkAPI, locality.NewZonedID(zone, ID), nil
}
...
func flattenBaremetalOptions(zone scw.Zone, options []*baremetal.ServerOption) interface{} {
	...
	for _, option := range options {
		flattenedOptions = append(flattenedOptions, map[string]interface{}{
			"id":         locality.NewZonedID(zone, option.ID).String(),
...
func flattenBaremetalPrivateNetworks(region scw.Region, privateNetworks []*baremetal.ServerPrivateNetwork) interface{} {
	...
	for _, privateNetwork := range privateNetworks {
		flattenedPrivateNetworks = append(flattenedPrivateNetworks, map[string]interface{}{
			"id":         locality.NewRegionalIDString(region, privateNetwork.PrivateNetworkID),
